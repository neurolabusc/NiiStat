function nii_stat_core(les,beh, beh_names,hdr, kPcrit, kNumRandPerm, kOnlyAnalyzeRegionsDamagedInAtleastNSubjects,statname, roi_names, hdrTFCE)
...
if isempty(roi_names) %voxelwise analysis
    saveSumMapSub(hdr,sum (les, 1),statname); %create image showing sum of values
else
...
if isempty(roi_names) %voxelwise
	reportResultsVoxel(z,[],threshMin,threshMax,good_idx,beh_names,hdr,statname); 
...
function reportResultsVoxel(z,c,threshMin,threshMax,good_idx,beh_names,hdr, statname)
...
function saveSumMapSub(hdr,sumImg,statname)
if isempty(hdr), return; end;
