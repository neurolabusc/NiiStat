function nii_stat_core2(les,beh, beh_names,hdr, kPcrit, kNumRandPerm, kOnlyAnalyzeRegionsDamagedInAtleastNSubjects,statname, roi_names, hdrTFCE, voxMask)
...
bf = zeros(numVox,numFactors); %pre-allocate observed Bayes factors
bfT1 = zeros(numVox,numFactors); %pre-allocate Type I error rates for Bayes factors
threshMax = zeros(numFactors,1); %pre-allocate statistical threhsold
threshMin = threshMax;  %pre-allocate statistical threhsold
threshMin_bf = threshMax;  %pre-allocate statistical threhsold
threshMax_bf = threshMax;  %pre-allocate statistical threhsold
...
elseif isBinomialBehav && isBinomialLes %binomial data
    ...
    for i = 1: size(beh,2)
        [z(:,i), threshMin(i), threshMax(i),bf(:,i),bfT1(:,i),threshMin_bf(i),threshMax_bf(i)] = glm_permSub(les,beh(:,i), kNumRandPerm, kPcrit, good_idx, hdrTFCE);
    end;
end
...
if (kNumRandPerm == -1) || (kNumRandPerm == -2) %report thresholds using FDR correction
    for i = 1:numFactors
        ...
        fprintf('The default standard of evidence for Bayes factors is BF>10, BF<.10\n');
    end
elseif kNumRandPerm == 0 %report thresholds using bonferroni correction
    ...
    for i = 1:numFactors
        ...
        fprintf('p<%.3f permutation correction for %s is BF<%.5f BF>%.5f\n',kPcrit,deblank(beh_names{i}),threshMin_bf(i), threshMax_bf(i));
    end;
end %else report permutation thresholds
%next: report results
if isempty(roi_names) %voxelwise
	reportResultsVoxel(z,[],threshMin,threshMax,good_idx,beh_names,hdr,'Z');%, voxMask); 
    reportResultsVoxel(bf,[],threshMin_bf,threshMax_bf,good_idx,beh_names,hdr,'BF');
%     if kNumRandPerm > 2
%         reportResultsVoxel(bf,[],-Inf,.04999,good_idx,beh_names,hdr,'BFT1');
% %         bf_thresh = bf .* (bfT1 < .05);
% %         reportResultsVoxel(bf_thresh,[],-Inf,10,good_idx,beh_names,hdr,'BF_T1thresh');
%     end
...
for i = 1:length(beh_names) %or size(beh,2)
    ...
    fprintf('%s %s=%f..%f, %d voxels survive threshold\n', deblank(beh_names{i}),statname,min(z(:,i)),max(z(:,i)),length(thresh_idx) );    
    if ~isempty(hdr)
        zMask = z(:,i);
        %if isempty(voxMask)
        ...
        %statImg = reshape(statImg,hdr.dim(1),hdr.dim(2),hdr.dim(3));
        saveStatMapSub(hdr,statImg,[statname, deblank(beh_names{i})], threshMin(i),threshMax(i) );   
        if ~isempty(thresh_idx)
            ...
            %end
            %statImg = reshape(statImg,hdr.dim(1),hdr.dim(2),hdr.dim(3));
            saveStatMapSub(hdr,statImg,[sprintf('thresh_%s', statname), deblank(beh_names{i})], threshMin(i),threshMax(i) );     
            if ~isempty(c)
                    cMask = c(:,i);
...
function [uncZ, threshMin, threshMax, bf, bfT1, threshMin_bf, threshMax_bf] = glm_permSub(Y, X, nPerms, kPcrit, good_idx, hdrTFCE)
%returns uncorrected z-score for all voxels in Y given single column predictor X
% Y: volume data 
% X: single column predictor
...
%%% Bayes Factor added by Grant M. Walker 10/2016 %%%
ndam = sum(Y);
bf = nan(v,1);
mypool = gcp;
if isempty(mypool); mypool=parpool(); end
tic;
fprintf('Computing Bayes factors ... ');
parfor kk=1:v;
    bf(kk) = t2smpbf(t(kk),ndam(kk),n-ndam(kk));
end
fprintf('required %.1f seconds.\n',toc);
...
startTime = tic;
bfT1 = zeros(v,1);
peak_bf = nan(nPerms,1);
nadir_bf = nan(nPerms,1);
sort_bf = sort(bf);
peak_bf(1) = sort_bf(length(bf)-round(length(bf) * 0.01));
nadir_bf(1) = sort_bf(round(length(bf) * 0.01));
for p = 2:nPerms
    if p == perm5pct
    ...
    bfp = nan(v,1);
    parfor kk=1:v;
        bfp(kk) = t2smpbf(tp(kk),ndam(kk),n-ndam(kk));
        if bfp(kk)>=10
            bfT1p(kk)=1;
        end
    end
    sort_bf = sort(bfp);
    peak_bf(p) = sort_bf(length(bf)-round(length(bf) * .01)); % .01 = false discovery rate
    nadir_bf(p) = sort_bf(round(length(bf) * .01));
    bfT1 = bfT1+bfT1p;
end
threshMin = permThreshLowSub (nadir, kPcrit);
threshMax = permThreshHighSub (peak, kPcrit);
threshMin_bf = permThreshLowSub (nadir_bf, .005); % 1-.005 = likelihood that criteria controls max FDR
threshMax_bf = permThreshHighSub (peak_bf, .005);
